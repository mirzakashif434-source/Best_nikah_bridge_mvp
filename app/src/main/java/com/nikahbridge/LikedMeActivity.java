...
/**
 * Real Azure "Liked Me" screen.
 * Reads only authenticated Azure like records; no demo or fabricated members.
 */
public class LikedMeActivity extends Activity {
    ...
    @Override protected void onCreate(Bundle state){
        ...
        PremiumFeatureGate.require(this,"whoLikedYou","20 SAR Basic or higher",this::build);
    }
    ...
    private void build(){
        ...
        TextView title=Premium2030Ui.title(this,"Liked Me");
        title.setGravity(Gravity.CENTER);
        root.addView(title);
        root.addView(Premium2030Ui.subtitle(this,
                "Real members who liked your profile • Azure verified data • No demo profiles"));
        root.addView(Premium2030Ui.heroLine(this,"See genuine interest, then choose respectfully"));

        status=Premium2030Ui.subtitle(this,"Loading real Azure likes…");
        ...
        refresh=Premium2030Ui.primary(this,"Refresh Liked Me");
    ...
    private void load(){
        ...
        status.setText("Loading real Azure likes…");
        list.removeAllViews();

        AzureApiClient.get("/likes/received",new AzureApiClient.Callback(){
            @Override public void ok(int code,String body){
                runOnUiThread(()->{
                    refresh.setEnabled(true);
                    refresh.setText("Refresh Liked Me");
                    try{
                        JSONObject response=new JSONObject(body);
                        JSONArray likes=response.optJSONArray("likes");
                        int count=response.optInt("incomingLikeCount",likes==null?0:likes.length());
                        status.setText(count==1?"1 real member liked you":count+" real members liked you");
                        ...
                        status.setText("Liked Me data could not be displayed.");
                    }
                });
            }
            ...
            @Override public void err(String message){
                runOnUiThread(()->{
                    ...
                    if(m.contains("PREMIUM_BASIC_REQUIRED")||m.contains("402")){
                        PremiumFeatureGate.showUpgrade(LikedMeActivity.this,"20 SAR Basic or higher");
                    }else if(m.contains("AZURE_SIGN_IN_REQUIRED")||m.contains("AZURE_INTERACTION_REQUIRED")||m.contains("401")){
                        status.setText("Azure sign in is required. Tap Refresh after signing in.");
                    }else{
                        status.setText("Liked Me is temporarily unavailable. Check your connection and tap Refresh.");
                    }
                });
            }
        });
    ...
    private void showEmpty(){
        ...
        card.addView(Premium2030Ui.section(this,"No new likes yet"));
        card.addView(Premium2030Ui.subtitle(this,
                "When a real member likes your profile, they will appear here automatically."));
        list.addView(card);
    }
    ...
    private void addLikeCard(JSONObject item){
        ...
        final boolean likedBack=item.optBoolean("liked_back",false);
        ...
        if(likedBack)details.append(details.length()>0?"\n":"").append("♥ You liked each other");
        TextView detailText=Premium2030Ui.subtitle(this,details.length()==0?"Real profile details available":details.toString());
    ...
    private void showProfile(String name,int age,String country,String city,String intention,
        ...
        body.append("\n\nOnly privacy-safe Azure profile fields are shown here.");
    ...
    private void likeBack(String userId,Button button){
        if(userId==null||userId.trim().isEmpty()){
            LanguageManager.toast(this,"This member cannot receive a like right now.",Toast.LENGTH_LONG).show();
            return;
        }
        try{
            ...
            AzureApiClient.post("/likes",body.toString(),new AzureApiClient.Callback(){
                @Override public void ok(int code,String response){
                    runOnUiThread(()->{
                        button.setText("♥ Liked Back");
                        button.setEnabled(false);
                        LanguageManager.toast(LikedMeActivity.this,
                                "Like sent securely. If both sides like each other, the mutual signal is recorded.",
                ...
                @Override public void err(String message){
                    runOnUiThread(()->{
                        ...
                        button.setText("Like Back");
                        if(message!=null&&message.contains("ROLLING_24H_LIKE_LIMIT_REACHED")){
                            LanguageManager.dialog(LikedMeActivity.this)
                            ...
                            LanguageManager.toast(LikedMeActivity.this,
                                    "Like could not be sent. Please try again.",
                                    Toast.LENGTH_LONG).show();
                        }
                    });
                }
