...
public class SmartSeriousQuestionsActivity extends Activity {
    ...
    private void render(){
        ...
        Button generate=btn("Generate Questions",true);root.addView(generate,new LinearLayout.LayoutParams(-1,dp(62)));generate.setOnClickListener(v->generate());
        ...
        AzureApiClient.get("/matches/"+id,new AzureApiClient.Callback(){
            public void ok(int code,String body){runOnUiThread(()->{
                try{
                    ...
                    root.addView(txt("Questions based on real stated match information",19,true));
                    ...
                    root.addView(txt("These are discussion prompts only. They do not determine character, safety, religious standing, or marriage success.",14,false));
                }catch(Exception e){toast("Azure match response could not be read.");}
            });}
            public void err(String message){runOnUiThread(()->toast("Could not load real Azure match: "+message));}
        });
    }
