...
public class AzureWalletActivity extends Activity {
    ...
    private void base(){root=new LinearLayout(this);root.setOrientation(LinearLayout.VERTICAL);root.setPadding(22,24,22,32);root.setBackgroundColor(Premium2030Ui.CREAM);ScrollView s=new ScrollView(this);s.addView(root);setContentView(s);}
    private TextView t(String x){TextView v=new TextView(this);v.setText(x);v.setTextSize(16);v.setTextColor(Premium2030Ui.TEXT);v.setPadding(6,10,6,12);return v;}
    ...
    @Override protected void onCreate(Bundle state){super.onCreate(state);base();root.addView(t("Azure Wallet"));TextView status=t("Loading real wallet…");root.addView(status);
        loadWallet(status);
        Button tx=b("View Real Transactions");tx.setOnClickListener(v->AzureApiClient.get("/wallet/transactions",new AzureApiClient.Callback(){
            public void ok(int code,String body){runOnUiThread(()->status.setText(body));}
        ...
        AzureApiClient.get("/wallet",new AzureApiClient.Callback(){
            public void ok(int code,String body){runOnUiThread(()->{
                try{
                    ...
                    status.setText(w==null?body:"Balance: "+w.optString("balance")+" "+w.optString("currency"));
                }catch(Exception e){status.setText(body);}
            });}
