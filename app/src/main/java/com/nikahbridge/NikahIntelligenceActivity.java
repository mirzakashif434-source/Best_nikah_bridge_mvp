...
public class NikahIntelligenceActivity extends Activity {
    ...
    private void save(){ Map<String,Object> x=new HashMap<>(); x.put("country",v(country)); x.put("city",v(city)); x.put("marriageTimeline",v(timeline)); x.put("familyInvolvement",v(family)); x.put("childrenExpectation",v(children)); x.put("careerPlan",v(career)); x.put("livingPlan",v(living)); Map<String,Object> profile=new HashMap<>(); profile.put("deenPriorities",v(deen)); profile.put("dealbreakers",v(dealbreakers)); profile.put("updatedAt",FieldValue.serverTimestamp()); fn.getHttpsCallable("updateLivingCompatibility").call(x).addOnSuccessListener(r->db.collection("users").document(auth.getUid()).set(profile,SetOptions.merge()).addOnSuccessListener(z->{toast("Marriage Blueprint saved securely.");calculate();}).addOnFailureListener(e->toast("Blueprint core saved, but profile preferences could not be saved."))).addOnFailureListener(e->toast("Could not save Blueprint. No local-only fake save was used.")); }
