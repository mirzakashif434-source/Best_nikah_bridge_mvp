...
public class MainActivity extends Activity {
    ...
    private void home(){
        ...
        Button back=button("Back",false);back.setOnClickListener(x->finish());
    ...
    private void chat(String cid,String other){
        ...
        send.setOnClickListener(x->{String msg=e.getText().toString().trim();if(msg.isEmpty()||msg.length()>4000){toast("Message must be 1–4000 characters.");return;}send.setEnabled(false);Map<String,Object> m=new HashMap<>();m.put("toUid",other);m.put("text",msg);functions.getHttpsCallable("sendConnectionMessage").call(m).addOnSuccessListener(y->{e.setText("");msgs.addView(text("You: "+msg,15,false));send.setEnabled(true);}).addOnFailureListener(y->{send.setEnabled(true);String message=y.getMessage();if(message==null||message.trim().isEmpty())message="Message could not be sent.";toast(message);});});
