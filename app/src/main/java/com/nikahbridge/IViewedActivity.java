...
/**
 * Real Azure "I Viewed" screen.
 * Shows only profiles the authenticated user explicitly opened.
 */
public class IViewedActivity extends Activity {
    ...
    private void build(){
        ...
        TextView title=Premium2030Ui.title(this,"I Viewed");
        title.setGravity(Gravity.CENTER);
        root.addView(title);
        root.addView(Premium2030Ui.subtitle(this,
                "Profiles you explicitly opened • Real Azure history • No demo views"));
        root.addView(Premium2030Ui.heroLine(this,"Return to profiles you genuinely reviewed"));

        status=Premium2030Ui.subtitle(this,"Loading your real Azure view history…");
        ...
        refresh=Premium2030Ui.primary(this,"Refresh I Viewed");
    ...
    private void load(){
        ...
        status.setText("Loading your real Azure view history…");
        list.removeAllViews();

        AzureApiClient.get("/profile-views/sent",new AzureApiClient.Callback(){
            @Override public void ok(int code,String body){
                runOnUiThread(()->{
                    refresh.setEnabled(true);
                    refresh.setText("Refresh I Viewed");
                    try{
                        JSONObject response=new JSONObject(body);
                        JSONArray viewed=response.optJSONArray("viewed");
                        int count=response.optInt("count",viewed==null?0:viewed.length());
                        status.setText(count==1?"You viewed 1 real profile":"You viewed "+count+" real profiles");
                        if(viewed==null||viewed.length()==0){
                            showEmpty();
                            return;
                        }
                        for(int i=0;i<viewed.length();i++){
                            JSONObject item=viewed.optJSONObject(i);
                            if(item!=null)addViewedCard(item);
                        }
                    }catch(Exception e){
                        status.setText("I Viewed data could not be displayed.");
                    }
                });
            }
            ...
            @Override public void err(String message){
                runOnUiThread(()->{
                    ...
                    if(m.contains("AZURE_SIGN_IN_REQUIRED")||m.contains("AZURE_INTERACTION_REQUIRED")||m.contains("401")){
                        ...
                        status.setText("I Viewed is temporarily unavailable. Check your connection and tap Refresh.");
                    }
                });
            }
        });
    ...
    private void showSignInRecovery(){
        list.removeAllViews();
        status.setText("Azure sign in is required to load your real view history.");
        ...
        signIn.setOnClickListener(v->{
            ...
            AzureAuthManager.acquireTokenInteractive(this,new AzureAuthManager.Callback(){
                @Override public void ok(String accessToken){runOnUiThread(IViewedActivity.this::load);}
    ...
    private void showEmpty(){
        ...
        card.addView(Premium2030Ui.section(this,"No viewed profiles yet"));
        card.addView(Premium2030Ui.subtitle(this,
                "When you explicitly open a real profile, it will appear here automatically."));
        list.addView(card);
    }

    private void addViewedCard(JSONObject item){
        ...
        details.append(details.length()>0?"\n":"").append("You opened this profile ").append(viewCount).append(viewCount==1?" time":" times");
        ...
        Button view=Premium2030Ui.secondary(this,"View Profile Again");
        card.addView(view,new LinearLayout.LayoutParams(-1,dp(52)));
        view.setOnClickListener(v->{
            ProfileViewTracker.record(IViewedActivity.this,userId);
            showProfile(name,age,country,city,intention,education,family,identityVerified,photoVerified);
        });

        Button like=Premium2030Ui.primary(this,iLiked?"♥ Liked":"Like Now");
            ...
            AzureApiClient.post("/likes",body.toString(),new AzureApiClient.Callback(){
                @Override public void ok(int code,String response){
                    runOnUiThread(()->{
                        ...
                        button.setEnabled(false);
                        LanguageManager.toast(IViewedActivity.this,"Like sent securely in Azure.",Toast.LENGTH_LONG).show();
                    });
                }
                @Override public void err(String message){
                    runOnUiThread(()->{
                        button.setEnabled(true);
                        button.setText("Like Now");
                        if(message!=null&&message.contains("ROLLING_24H_LIKE_LIMIT_REACHED")){
                            LanguageManager.dialog(IViewedActivity.this)
                                    .setTitle("24-hour like limit reached")
                                    .setMessage("You can send up to 20 likes in any rolling 24-hour period.")
                                    .setPositiveButton("OK",null)
                                    .show();
                        }else{
                            LanguageManager.toast(IViewedActivity.this,"Like could not be sent. Please try again.",Toast.LENGTH_LONG).show();
                        }
                    });
                }
            });
