    ...
    private void load(){
        ...
        AzureApiClient.get("/admin/owner/live-analytics",new AzureApiClient.Callback(){
            ...
            public void err(String m){runOnUiThread(()->{
                if(m!=null&&m.contains("ADMIN_REQUIRED"))status.setText("Owner access is not enabled for this Azure account.");
                else if(m!=null&&(m.contains("401")||m.contains("AZURE_AUTH")))status.setText("Azure owner session needs sign-in again.");
                else status.setText("Owner analytics unavailable: "+m);
            });}
        });
    }
