...
public class WelcomeActivity extends Activity {
    ...
    private void showWelcome() {
        ...
        scroll.setFillViewport(true); scroll.setBackgroundColor(cream);
        root = new LinearLayout(this); root.setOrientation(LinearLayout.VERTICAL);
        root.setGravity(Gravity.CENTER_HORIZONTAL); root.setPadding(28,28,28,28);
        scroll.addView(root); setContentView(scroll);

        TextView arabic=text("بِسْمِ اللَّهِ الرَّحْمَنِ الرَّحِيمِ",30,false,dark); arabic.setGravity(Gravity.CENTER);
        root.addView(arabic,params(-1,-2,0,8,0,4));
        TextView welcome=text("Welcome",39,true,dark); welcome.setGravity(Gravity.CENTER); root.addView(welcome,params(-1,-2,0,6,0,6));
        TextView bridge=text("♡  BEST  NIKAH BRIDGE  ♡",25,true,green); bridge.setGravity(Gravity.CENTER); root.addView(bridge,params(-1,-2,0,4,0,14));
        TextView tagline=text("Meaningful Matches    •    Verified Profiles    •    Private & Safe",15,true,dark); tagline.setGravity(Gravity.CENTER); root.addView(tagline,params(-1,-2,0,8,0,18));
        TextView urdu=text("ایک معتبر مسلم رشتہ پلیٹ فارم\nحلال نکاح • اعتماد • خاندان • رازداری",17,false,dark); urdu.setGravity(Gravity.CENTER); root.addView(urdu,params(-1,-2,0,8,0,20));
        Button getStarted=button("GET STARTED\nآغاز کریں",true); Button signIn=button("SIGN IN\nلاگ اِن کریں",false);
        root.addView(getStarted,params(-1,dp(72),0,8,0,8)); root.addView(signIn,params(-1,dp(72),0,8,0,8));
        TextView footer=text("Serious Muslim matrimonial platform — not a dating app.",14,false,dark); footer.setGravity(Gravity.CENTER); root.addView(footer,params(-1,-2,0,18,0,8));
        getStarted.setOnClickListener(v->showAccountDialog(false)); signIn.setOnClickListener(v->showAccountDialog(true));
    }
