...
public class ConversationHealthActivity extends Activity {
    ...
    private void render() {
        ...
        root.addView(txt("A factual view of your mutual connection: recorded message volume, replies and recent activity. It does not judge personality or relationship quality.", 15, false));
    ...
    private void showConnections(com.google.firebase.firestore.QuerySnapshot snapshot, String uid) {
        if (snapshot.isEmpty()) {
        ...
        db.collection("connections")
                ...
                .addOnSuccessListener(messages -> {
                    ...
                    for (DocumentSnapshot message : messages.getDocuments()) {
                        ...
                        if (uid.equals(sender)) mine++; else theirs++;
                        Timestamp ts = message.getTimestamp("createdAt");
                        if (ts != null) last = ts.toDate();
                    }
                    String status = theirs > 0 ? "Replies recorded" : "No reply recorded in the loaded messages";
                    String lastText = last == null ? "Not available" : last.toString();
                    root.addView(txt("Connection: " + connectionId
                            + "\nMessages reviewed: " + total
                            + "\nYour messages: " + mine
                            + "\nOther member messages: " + theirs
                            + "\nStatus: " + status
                            + "\nLast message: " + lastText, 16, true));
                })
