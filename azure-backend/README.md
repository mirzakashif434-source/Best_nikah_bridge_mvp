...
- Step 15 — Real authenticated account-deletion API with PostgreSQL cascade deletion, private Azure Blob cleanup for profile photos and verification documents, Firebase Auth cleanup, and live Function App verification: LOCKED after successful Azure Functions deployment run #85 (commit 3ec802521548f9d38c65299abf341c4e1dcdcfd2).
- Step 16 — Real authenticated admin/moderator safety & moderation center with report queue, report status workflow, user suspension/restore, and automatic profile hiding for suspended users: LOCKED after successful Azure Functions deployment run #88 (commit 511f4a89e905df38d4f192191d41663676bbc6e6).
- Step 17 — Real public account-deletion web resource plus authenticated external deletion endpoint, using the existing production deletion/storage cleanup: LOCKED after successful Azure Functions deployment run #91 (commit 3a40a816c4b6fa12f9b641eaa57fc84bd34f5622).

- Firebase migration #1 — Azure Entra External ID authentication foundation: ADDITIVE IMPLEMENTATION ADDED. Includes production JWT/JWKS validation, PostgreSQL Azure identity mapping, and authenticated /api/auth/azure/me. Firebase Auth remains untouched. NOT LOCKED until an actual Azure External ID external tenant, app registration/user flow, API audience, JWKS issuer, Android MSAL integration, live sign-in, and Azure deployment verification are completed.

- Firebase migration #1 — Azure Entra External ID authentication: FOUNDATION VERIFIED IN REPOSITORY. Azure JWT/JWKS validation, PostgreSQL Azure identity mapping, and authenticated Azure auth endpoint are present. Firebase Auth remains untouched. **NOT LOCKED**: external tenant/app registration, real user flow, Android MSAL client integration, production token acquisition, live API authentication, and deployment verification are still required before this migration can be locked.
