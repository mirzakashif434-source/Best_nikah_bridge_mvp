...
app.http("conversationsList",{
  ...
  handler:requireAuth(async(request,context,user)=>{
    try{
      ...
      const r=await query(
        "SELECT c.id,c.status,c.created_at,CASE WHEN c.user_a_id=$1 THEN ub.firebase_uid ELSE ua.firebase_uid END AS other_user_id,CASE WHEN c.user_a_id=$1 THEN pb.display_name ELSE pa.display_name END AS other_display_name,(SELECT m.body FROM messages m WHERE m.conversation_id=c.id ORDER BY m.created_at DESC LIMIT 1) AS last_message,(SELECT m.created_at FROM messages m WHERE m.conversation_id=c.id ORDER BY m.created_at DESC LIMIT 1) AS last_message_at FROM conversations c JOIN users ua ON ua.id=c.user_a_id JOIN users ub ON ub.id=c.user_b_id LEFT JOIN profiles pa ON pa.user_id=ua.id LEFT JOIN profiles pb ON pb.user_id=ub.id WHERE c.status='mutual' AND (c.user_a_id=$1 OR c.user_b_id=$1) ORDER BY COALESCE((SELECT m.created_at FROM messages m WHERE m.conversation_id=c.id ORDER BY m.created_at DESC LIMIT 1),c.created_at) DESC LIMIT 200",
        [me.id]);
...
app.http("messagesList",{
  ...
  handler:requireAuth(async(request,context,user)=>{
    try{
      ...
      const r=await query(
        "SELECT m.id,m.conversation_id,u.firebase_uid AS sender_user_id,m.body,m.created_at,m.read_at FROM messages m JOIN users u ON u.id=m.sender_user_id WHERE m.conversation_id=$1 ORDER BY m.created_at ASC LIMIT 500",
        [id]);
