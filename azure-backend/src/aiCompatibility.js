...
const MAX = Number(process.env.AI_ADVANCED_MAX_INPUT_CHARS || 4000);
...
app.http("aiNikahMediator",{methods:["POST"],authLevel:"anonymous",route:"ai/nikah-mediator",handler:requireAuth(async(req,context,user)=>{try{const a=await accessForAuth(user);if(!a.capabilities.advancedMatching)return {status:402,jsonBody:{ok:false,error:"PREMIUM_PLUS_REQUIRED",locked:true}};const quota=await consumeAiQuota(a.user.id,a.premium.planKey,"advanced_matching");if(!quota.allowed)return quotaResponse(quota);const result=await runAzureAI(await req.json(),mediatorInstruction,responseLanguage(req));if(result?.jsonBody?.ok)result.jsonBody.aiQuota={dailyLimit:quota.limit,remaining:quota.remaining};return result;}catch(e){return {status:e.statusCode||500,jsonBody:{ok:false,error:e.statusCode?e.message:"AI_MEDIATOR_FAILED"}};}})});
app.http("aiFutureLife",{methods:["POST"],authLevel:"anonymous",route:"ai/future-life",handler:requireAuth(async(req,context,user)=>{try{const a=await accessForAuth(user);if(!a.capabilities.advancedMatching)return {status:402,jsonBody:{ok:false,error:"PREMIUM_PLUS_REQUIRED",locked:true}};const quota=await consumeAiQuota(a.user.id,a.premium.planKey,"advanced_matching");if(!quota.allowed)return quotaResponse(quota);const result=await runAzureAI(await req.json(),futureInstruction,responseLanguage(req));if(result?.jsonBody?.ok)result.jsonBody.aiQuota={dailyLimit:quota.limit,remaining:quota.remaining};return result;}catch(e){return {status:e.statusCode||500,jsonBody:{ok:false,error:e.statusCode?e.message:"AI_FUTURE_LIFE_FAILED"}};}})});
